...
// Parses the standard "Netscape Bookmark File" HTML format used by
// Raindrop.io, Chrome, Firefox and most other bookmark managers'
// export features. The markup is technically malformed HTML (unclosed
// <DT> tags), so browsers' HTML parser flattens nested <DL>/<DT> into
// siblings — we walk that flattened structure and reconstruct folders
// (<H3>) as groups, using a " / " path for nested subfolders.
function parseNetscapeBookmarks(html) {
  const doc = new DOMParser().parseFromString(html, 'text/html');
  const rootDl = doc.querySelector('dl');
  if (!rootDl) throw new Error('Formato de archivo inválido');

  const groups = [];

  function walkDl(dl, parentPath) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (h3) {
        const folderName = h3.textContent.trim() || 'Sin título';
        const fullName = parentPath ? `${parentPath} / ${folderName}` : folderName;
        const groupEntry = { name: fullName, bookmarks: [] };
        groups.push(groupEntry); // push before recursing so parent folders precede their subfolders
        const next = children[i + 1];
        if (next && next.tagName === 'DL') {
          groupEntry.bookmarks = walkDl(next, fullName);
          i++;
        }
      } else if (a) {
        const url = a.getAttribute('href');
        if (url) looseBookmarks.push({ title: a.textContent.trim() || url, url });
      }
    }

    return looseBookmarks;
  }

  const rootBookmarks = walkDl(rootDl, '');
  if (rootBookmarks.length > 0) {
    groups.unshift({ name: 'Marcadores importados', bookmarks: rootBookmarks });
  }
  return groups;
}
...
export async function importData(text) {
  ...
  if (trimmed.startsWith('<')) {
    incoming = parseNetscapeBookmarks(trimmed);
  } else {
